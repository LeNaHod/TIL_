Python TIL1(22_09_06 ~ 22_09_07)

# [1] 데이터 변수와 데이터형식에 대해알아보자
- 데이터 : 자료.DATUM의 복수형이다. 자료형 = 데이터 타입 = 자료 유형
  진법으론 2진수,8진수,10진수,16진수, 수체계는 정수,실수(부동소수점 수)
- 데이터 처리 : 정보->자료 / 자료->정보 데이터를 한 가지 형식에서 다른형식으로 바꾼다.
  데이터 처리안에 데이터 분석이있다. 주로 정보생산, 지식생산등에 이용되며, '정보 처리 시스템'이라고도 불림
- 변수 : 데이터를 담는 바구니같은것. 메모리 장치 내의 특정부분을 가리키는 지시기(Pointer포인터)에대한 별명
  포인터에는 메모리의 특정 부분에 대한 주소가 담기는데 해당 메모리 부분에 데이터가 저장되는 원리이다. 그걸
  우리가 변수명으로 불러오는것
- 데이터 형식(data type) : 데이터 형식 = 자료형 =자료유형 = 데이터 타입. 컴퓨터가 자료를 저장하는방식이다.
- ★ 숫자형 float 과 double : 두개 다 실수를 표현하지만 2진수로 저장하기에, 무한하게 순환하는 소수같은 경우는
  저장자릿수가 넘으면 남은 자릿수는 버려지게되므로 **버려지는 만큼, 오차**가 생기게된다. double형은 저장할 수 있는 자릿수가 더 많지만 똑같이 무한하지 않기때문에, float보다 오차가 적다는 정도가 차점이다. 고로 꼭 써야한다면 반올림해주던가 해야한다.
- 나누기 = % / result = 종료값(반환함) / a=a+1 == a +=1 / **is붙은건 보통 판별용** isnull=null값인가?isupper=대문자인가? islower=소문자인가?

## 각종 예제 코드(숫자,문자열,연산)

<details>
<summary>변수,숫자,연산,문자열(리스트,슬라이싱 등등)</summary>
...
# 8진수와 16진수 표현.
a = 0o17      # 8진수. 공오처럼 o자를 대문자로 써도 되지만 0과 혼동되므로 소문자 사용이 관례.
b = 0xA1      # 16진수. 공엑스처럼 써도 되지만 대문자끼리 구분하기 힘들므로 소문자 사용이 관례.
print(a)      # 8진수 0o17은 10진수로는 15.
print(b)      # 16진수 0xA1은 10진수로는 161.
print(a + b)  # 진법이 달라도 연산을 할 수 있다

"""### 4.2. ★문자열★
가장 기본적이면서도 인공지능, 자연어 처리에 필수!
"""
...
포맷팅이란 문자열에 서식을 부여하는 일을 말한다. 예를들어 2022 10 11 를 'yyyy-mm-dd'라고 서식을주면 
2022-10-11 이나온다.
"""
...
새_문자열[0]  # 그렇다면 0번 문자열은 첫 번째 문자열일 것이다. '인'출력

새_문자열[29]

새_문자열[-1]  # 거꾸로 세어 올 때는 -1이 뒤에서 첫 번째 문자열에 해당한다. '.'출력

새_문자열[-3] # 따라서 뒤에서 세 번째 문자열을 -3으로 지정해야 한다.

"""#### 4.2.5 슬라이싱(slicing)
슬라이싱이란 문자열의 일부를 복제함. 원래 있던 문자열은 그대로
"""
...
문자열 함수란 문자열 자체에서 제공하는 함수. 문자열.함수명() 형태를가지고있다. 머신러닝 딥러닝에
사용
"""
...
# index 함수로도 문자의 위치를 알아낼 수 있다.
print(새_문자열.index('을')) # 을이 처음으로 나온 위치를 알려준다.
# print(새_문자열.index('뙳')) # 찾는 문자열이 없으면 오류를 출력한다.

# 특정 문자를 문자열을 이루는 문자들 사이에 끼우기
print(" ".join("아버지가방에들어가신다."))
...
print("아버지가,방에,들어가신다.".split('에')) # 지정한 구분자를 기준으로 문자열을 나눈다.

"""### 4.3 리스트(list)
목록형식으로 자료를 나열해놓은것. 나열형자료형. 
리스트같은 형식을 컨테이너라고도 부르는데 리스트는 컨테이너의 일종이다.
...
"""### 4.4 튜플(tuple)
리스트와 비슷한 특징을 가지고있으나, ★한번 저장하면 수정할수없다★ 이뮤터블(변경 불가)형식
()=튜플 []=리스트 {}=딕셔너리 set=set((),[])
"""
...
"""### 4.5 셋(set)
이 데이터 형식은 수학에 나오는 집합(set) 연산을 처리하기 쉽게 하기 위한 것
#### 4.5.1 셋 형식의 특징
> 수학에 나오는 집합의 특징과 동일
* 중복을 허용하지 않음
* 순서를 고려하지 않음
...
# 문자열을 셋으로 만들기  
문자_집합 = set("반갑다, 세상아, 다시 반갑다!") # 순서가 사라져 버린다.
print(문자_집합)
...
전화부 = {'김갑돌': '010-1234-5678',
          '이을돌': '010-2345-6789'}
print(전화부)   > 김갑돌: 010-1234-5678 이을돌 : 010-2345-6789 식으로나옴
...
print('김갑돌' in 성적부)
print('이을돌' in 성적부)  # 성적부 '안에 있는' 이을돌 '을 출력해라.'
...
--------------------
# [2] Python의 IF,CASE,WHILE,FUNCTION1
If의 기본형식. 줄맞추기를 잘 해야한다.

    IF조건문 :
        명령문(들여쓰기 필수)
    ELIF :                               = ELSE IF
        명령문 
    ELSE :                               = END IF 가 없다. 대신 IF의 줄 밖으로나가면 END IF격침
        명령문                            = ELSE가 필수가아니다. 알아서 조건외 경우를 FALSE처리

Case의 기본형식.  브레이크문이없다

    match 객체변수:                 객체위치에따라 케이스문이 달라질수있음
        case 1: 명령문
        case 2: 명령문
        case _: 명령문

While의 기본형식. 무한반복을하고싶을때 =입력용 (FOR는 어디서 어디까지 반복 =출력용)
...
<details>
<summary>IF,CASE,WHILE,FUNCTION1 예제</summary>
...
match su:
    ...
    case 2 :print('2입니다')
    case 3 :print('3입니다')
    case _ : print('else는이쪽')


# case는 튜플과 리스트의 값을 인덱스값으로 판단한다.

my_list = '1,2,3,4'   결과는 else쪽으로빠진다.
match my_list:
    case [1,2,3]: print('1입니다')
    case (1,2,3,4): print('2입니다')
    case 3 :print('3입니다')
    case _ : print('else는이쪽')

# 혼합형은 따로들어간다. x에 3이들어가고 others에 리스트값.
...
match my_list:
    ...
    case (1,2,3,4) :print('2입니다')
    case 3 :print('3입니다')
    case 1,[x,others] : print(f'else는이쪽 {x} {others}')  이결과가나옴


#시퀀스값일때(튜플,리스트,딕셔녀리)선택적으로(트루,펄스)출력하는 구문도 만들수있다.
my_su = [10,10]
match my_su:
    case [x,y] if x >5 and y < 5 : print('1입니다')
    case x,y if x>50 | y<50: print('2입니다')
    case _: print('안녕하세요')
...
cnt=0
while var > 0 :
    print(f'{var}',end='\t')    #f는 포맷이고 f다음에 ''안에 {}나 형식이있다면 문자열을
                                #이 양식대로 쓰겠다는소리 {b}는 b의값을바로가져옴
    var -=1
    cnt +=1
    print(cnt)
...
b=0
while a <100 :
    if a % 2 != 0 :  #나눈값이 0이아닐때 즉, 홀수일때
        b +=1
        print(a)    #홀수일때만 a를 출력
    a +=1           #그리고 출력을하지않지만 100이될떄까지 +1
else :              
    print('b=',b)  
...
if __name__ == '__main__' :
    test04()                   # 결과는 아무것도안나온다 test4가 pass라서
...
def getA():
    print('----')
    return b   #리턴은 함수종료키워드 결과값을 지정한곳에 저장해줌
